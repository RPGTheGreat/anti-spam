...
module.exports = async (client, options) => {
 ...
 client.on("checkMessage", async (message) => {
  ...
  const WarnMember = async (m, reply) => {
    ...
    if (message.member.roles.cache.some(r => ignoredRoles.includes(r.name)) || ignoredMembers.includes(message.author.tag) || message.guild.channels.cache.some(c => ignoredChannels.includes(c.name)) || ignoreBots && message.author.bot || message.member.hasPermission(p => ignorePermissions(p)) || message.client.user.id || message.guild.ownerID) return
